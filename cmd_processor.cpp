...
bool should_quit(string choice) { return (choice == "q" || choice == "quit"); }
...
bool AddTaskOp(TaskList& task_list, list<string> cmd) {
  ...
  while (it != cmd.end()) {
    if (*it == "-n") {
      ...
      {
        cout << "The begin_time parameter does not meet the requirements"
             << endl;
        return false;
      }
...
bool ModifyTaskOp(TaskList& task_list, list<string> cmd) {
  ...
  if ((flag1 && flag2 && it1 == it2) || (flag1 && name == "-" && !flag2) ||
      (!flag1 && id == -1 &&
       flag2))  // 3种符合查找要求的情况,分别是:id,name均有筛选且匹配;以及id,name仅有一个且找到
  {
...
bool SearchTaskOp(TaskList& task_list, list<string> cmd) {
  ...
  bool flag1 = it1 != task_list.return_end();
  bool flag2 = it2 != task_list.return_end();
  if ((flag1 && flag2 && it1 == it2) || (!flag1 && id == -1 && flag2) ||
      (flag1 && name == "-" && !flag2))  // 3种符合查找要求的情况,输出任务信息
  {
    if (flag1)
    ...
    cout << "The id and name used for searching do not meet the requirements"
         << endl;
...
// 使用说明:showtask -S start -E end -P priority -T type -a(show all) -A
...
bool ShowTaskOp(TaskList& task_list, list<string> cmd) {
  ...
  while (it != cmd.end()) {
    if (*it == "-a") {
      ...
      if (it1 == cmd.end() || *it1 == "-S" || *it1 == "-E" || *it1 == "-P" ||
          *it1 == "-T" || *it1 == "-A" || *it1 == "-D")
        flagForAll = true;
      else
        return false;
      ...
      if (str == "-E" || str == "-P" || str == "-T" || str == "-A" ||
          str == "-D" || str == "-a")  //-S后不跟参数,这是可以的
        continue;
      if (!isValidDate(str))  // 时间参数不符合要求
      {
        cout << "The start_time parameter does not meet the requirements"
             << endl;
        return false;
      }
...
bool AddTaskNoOp(TaskList& task_list) {
  ...
  // 添加任务的开始时间
  std::cout
      << "Please input the begin time of the task\nEnter q or quit to quit!"
      << endl;
  ...
  newtask.begin_time =
      to_time_t(begin_t);  // 将用户输入的形式转化成time_t格式保存
  // 添加任务的优先级
  std::cout << "Please input the priority of the task" << endl;
  std::cout << "-1-quit addtask, 1-low, 2-medium, 4-high, default value or "
               "other value-low"
            << endl;
  ...
  while (!std::cin) {
    ...
    std::cout
        << "Invalid input. Please enter a valid priority: \nEnter -1 to quit!"
        << endl;
    std::cin >> newtask.priority;
  ...
  if (newtask.priority != 1 && newtask.priority != 2 && newtask.priority != 4)
    newtask.priority = 1;
  // 添加任务的种类
  std::cout << "Please input the type of the task, such as: entertainment  "
               "sport  study  routine"
            << endl;
  std::cout
      << "If you want a default type, please enter -.\nEnter q or quit to quit!"
      << endl;
  ...
  // 添加任务的提醒时间
  std::cout
      << "Please input the remind time of the task\nEnter q or quit to quit!"
      << endl;
  std::cout << "Time format should be 2022/02/02/03:00:00" << endl;
  string remind_t;
  valid_input = false;
  ...
  newtask.remind_time =
      to_time_t(remind_t);  // 将用户输入的形式转化成time_t格式保存
  // id和开始时间一样
...
bool DeleteTaskNoOp(TaskList& task_list) {
  ...
  std::cout << "-1 represents quit\n1 represents deletion based on ID\nwhile "
               "the remaining represents deletion based on name"
            << endl;
...
bool ShowTaskNoOp(TaskList& task_list) {
  ...
  std::cout << "If you need to select according to the importance of the task, "
               "input 1"
            << endl;
  std::cout
      << "If you need to select according to the time of the task, input 2"
      << endl;
  std::cout << "If you need to select according to both the time and "
               "importance of the task, input 3"
            << endl;
  ...
  switch (option) {
    ...
    case 1:
      ...
      std::cout
          << "3 for low and medium, 6 for medium and high, 5 for low and high"
          << endl;
      ...
      break;
    case 2:
      std::cout << "If you want to see all the tasks before the specified "
                   "date, enter y/n"
                << endl;
      ...
      if (startTimeStr != "") startTime = to_time_t(startTimeStr);

      std::cout << "If you want to see all the tasks after the specified date, "
                   "enter y/n"
                << endl;
      std::cout << "format: 2022/02/02/10:00:00" << endl;
      std::cout << "Enter q or quit to quit!" << endl;
      ...
      if (endTimeStr != "") endTime = to_time_t(endTimeStr);
...
bool ModifyTaskNoOp(TaskList& task_list) {
  ...
  std::cout << "-1 represents quit\n1 represents search based on ID\nwhile the "
               "remaining represents search based on name"
            << endl;
  int choice;
  std::cin >> choice;
  if (choice == -1) {
