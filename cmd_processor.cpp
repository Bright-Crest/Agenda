...
bool AddTaskNoOp(TaskList& task_list) {
  ...
  // 检查优先级是不是符合1，2，4
  while (newtask.priority != 1 && newtask.priority != 2 &&
         newtask.priority != 4) {
    std::cout << "Invalid priority. Please try again." << endl;
    std::cin >> newtask.priority;
  }
  // 添加任务的种类
  std::cout << "Please input the type of the task：entertainment  sport  study "
               " routine"
            << endl;
...
bool DeleteTaskNoOp(TaskList& task_list) {
  ...
  std::cout << "1 represents deletion based on ID\nwhile the remaining "
               "represents deletion based on name"
            << endl;
...
bool ModifyTaskNoOp(TaskList& task_list) {
  ...
  std::cout << "1 represents search based on ID\nwhile the remaining "
               "represents search based on name"
            << endl;
  int choice;
  std::cin >> choice;
  while (!std::cin)  // 检查输入是否有效
